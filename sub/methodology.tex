...
\begin{figure}[t]
	\begin{center}
		\begin{tabular}{cc}
			\includegraphics[width=0.45\textwidth]{sonicator}&
			\includegraphics[width=0.45\textwidth]{spin_coating}
		\end{tabular}
	\end{center}
...
\begin{figure}[t]
	\begin{center}
		...
		\end{tabular}
	\end{center}
	\caption{OM(광학현미경)으로 찍은 silicon wafer의 모습.}
	\label{fig:om}  
\end{figure}

\subsection{데이터 추출}
제작된 sample을 NT-MDT 기기를 통하여 PL mapping 하였다. PL mapping이란, sample의 각 위치에서의 PL 데이터를 모두 담은 파일을 만드는 과정이다. 이렇게 만들어진 파일에서는 임의의 점에서의 PL data 를 얻어낼 수 있다는 장점이 있다.
...
\begin{figure}[t]
	\begin{center}
		...
		\end{tabular}
	\end{center}
	\caption{Nova-Px 프로그램에서 데이터를 추출하는 모습.}
	\label{fig:nova}  
\end{figure}
...
각 점들의 추출된 data를 분석하기 위해서는 Origin 9 프로그램을 사용하였다. (인용) chen 2018 에 의하면 CsPbBr3 에서 biexciton과 exciton peak이 나타나는 wave length 는 각각 약 580nm, 600nm 이다. 이 사실을 바탕으로 PL data에서 보여진 peak을 두개의 peak의 합으로 fitting 하였다. Peak fitting 을 할 때에 gauss 매커니즘을 사용하였으며, biexciton 과 exciton이 존재하는 wave length에 peak 위치를 설정한 후 fitting을 진행하였다. Figure \ref{fig:point0}은 그 중 하나의 예시이다.
\begin{figure}[t]
	\begin{center}
		\begin{tabular}{cc}
		...
		\end{tabular}
	\end{center}
	\caption{설정한 point 0 의 PL data를 두 개 peak 의 합으로 나타내어 fitting 한 모습.}
	\label{fig:point0}  
\end{figure}
